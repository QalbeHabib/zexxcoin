...
import { MonsterPresale } from "../target/types/monster_presale";
import {
  PublicKey,
  Connection,
  Keypair,
...
describe("palm-presale", () => {
  ...
  const program = anchor.workspace.MonsterPresale as Program<MonsterPresale>;
  ...
  it("Mint token to admin wallet", async () => {
    ...
    console.log((await connection.getBalance(adminPubkey)) / LAMPORTS_PER_SOL);
  ...
  it("Claim token!", async () => {
    ...
    const tx = await program.methods
      ...
      .accounts({
        tokenMint: mint,
        tokenAccount: buyerPresaleTokenAssociatedTokenAccount,
